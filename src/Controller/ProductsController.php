...
class ProductsController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            $this->Flash->error(__('El producto no pudo ser creado, por favor intente nuevamente.'));
        }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            $product = $this->Products->patchEntity($product, $this->request->getData());
            if ($this->Products->save($product)) {
                $this->Flash->success(__('El producto fue modificado correctamente.'));

                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error(__('El producto no pudo ser modificado, por favor intente nuevamente.'));
        }
        $this->set(compact('product'));
        $this->set('_serialize', ['product']);
    }
